...
const ProfilePage = ({ google, currentUser, foods, setFoods, showMap, setShowMap, openMap, closeMap }) => {
    ...
    useEffect(() => {
        axios({
            ...
            .then(response => {
                ...
                setTitle(`User Profile: ${response.data.name.charAt(0).toUpperCase() + response.data.name.slice(1)}`)
                setSmallTitle(`${response.data.name.charAt(0).toUpperCase() + response.data.name.slice(1)}\'s Reviews:`)
            })
            .catch(error => {
                console.log(error);
            });
